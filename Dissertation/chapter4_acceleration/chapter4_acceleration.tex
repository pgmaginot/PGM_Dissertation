...
The S2SA operator re-uses a lot of the transport sweep capability we have already developed with our high order DFEM of \secref{sec:chapter3_variable_xs}, but is computationally challenging to invert (in multiple spatial dimensions), and the MIP DSA operator we define in \secref{sec:dsa} requires significant derivation independent of the DFEM neutron transport methodology we have already derived, but is computationally efficient to invert.
...
However, since S2SA is really only practical for small, one dimensional problems, we will apply a variant of MIP DSA to accelerate the iterative process required for thermal radiative transfer problems in \secref{sec:chapter6_grey_radtran}.
