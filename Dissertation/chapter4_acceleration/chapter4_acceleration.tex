...
\caption{Estimates of $\rho$ for MIP DSA as a function of  $S_N$ order for $c=0.999$ linear SL Gauss.}
...
\caption{Estimates of $\rho$ for MIP DSA as a function of  $S_N$ order for $c=0.999$ linear SL Lobatto.}
\label{fig:mip_lobatto_as_fun_sn}
\end{figure}
%
%
As with S2SA, the higher $S_N$ order, the larger $\rho$ is for the MIP DSA scheme. 
However, the value of $\rho$ obtained with $S_8$ is only slightly smaller than the $\rho$ of $S_{16}$, and as such we prefer the reduced computational work of $S_8$ simulations to $S_{16}$ in examining iterative effectiveness.  Figures \ref{fig:mip_gauss_as_fun_sn}-\ref{fig:mip_lobatto_as_fun_sn} also exhibit $\rho$ dips associated with insufficient iteration to converge to the correct $\rho$.  
...
\caption{Estimates of $\rho$ for MIP DSA as a function of $c$  for  $S_8$  linear SL Gauss.}
...
\begin{figure}[!hbp]
\centering
\includegraphics[width=11cm]{chapter4_acceleration/Const_2_bar_Constant_XS_c_comparions_MIP_Gauss.png}
...
\begin{figure}[!htp]
\centering
\includegraphics[width=11cm]{chapter4_acceleration/Const_2_bar_Constant_XS_c_comparions_MIP_Lobatto.png}
...
Estimations of $\rho$ for MIP DSA as a function of $c$ for SL Gauss and SL Lobatto, with $Z_{MIP}=4$ are given in \fig{fig:mip_gauss_as_fun_c_z4} and \fig{fig:mip_lobatto_as_fun_c_z4}, respectively.
\begin{figure}[!htp]
\centering
\includegraphics[width=11cm]{chapter4_acceleration/Const_4_Constant_XS_c_comparions_MIP_Gauss.png}
\caption{Estimates of $\rho$ for MIP DSA as a function of $c$  for  $S_8$  linear SL Gauss, with $Z_{MIP}=4$.}
\label{fig:mip_gauss_as_fun_c_z4}
\end{figure}
\begin{figure}[!hbp]
\centering
\includegraphics[width=11cm]{chapter4_acceleration/Const_4_Constant_XS_c_comparions_MIP_Lobatto.png}
\caption{Estimates of $\rho$ for MIP DSA as a function of $c$  for  $S_8$  linear SL Lobatto, with $Z_{MIP}=4$.}
...
With $Z_{MIP}=4$, SL Lobatto displays a modest, smooth increase in $\rho$ over a large range of $\Sigma_t \Delta x$, but eliminates the large spikes in $\rho$ present in the optically thick regions of $\Sigma_t \Delta x$ for $Z_{MIP}=2$ in \fig{fig:mip_lobatto_as_fun_c}.
To determine whether the small spikes in $\rho$ for $\Sigma_t \Delta x \geq 100$ in \fig{fig:mip_gauss_as_fun_c_z4} is a spurious result of our iterative process, we plot the average $\rho$ achieved during the iteration process for SL Gauss with $Z_{MIP}=4$ in \fig{fig:mip_gauss_as_fun_c_z4_bar}.
\begin{figure}[!htp]
\centering
\includegraphics[width=11cm]{chapter4_acceleration/Const_4_bar_Constant_XS_c_comparions_MIP_Gauss.png}
...
%\end{figure}
%From \figs{fig:varxs_s2sa_gauss}{fig:varxs_mip_lobatto}, we see that both S2SA and MIP DSA remain effective iterative acceleration techniques for solve the $S_N$ neutron transport equations, despite significant corss section variation within each cell.
This is true so long as the S2SA and MIP DSA operators are differenced using the same trial space degree and quadrature as the underlying neutron transport discretization, be it SLXS Gauss or SLXS Lobatto.

\section{Conclusions}

S2SA is a more effective 
