...
There are small, but important differences between \figs{fig:mip_gauss_as_fun_c}{fig:mip_lobatto_as_fun_c} and \figs{fig:mip_gauss_as_fun_c_z4}{fig:mip_lobatto_as_fun_c_z4} .  With $Z_{MIP}=4$ in \fig{fig:mip_gauss_as_fun_c_z4} SL Gauss maintains the optically thin limit of $\rho$ for larger values of $\Sigma_t \Delta x$, smoothing out the spike near $\Sigma_t \Delta x=10$, and does not display as large of a spike near $\Sigma_t \Delta x =100$.
...
Figures \ref{fig:mip_gauss_c2_bar}-\ref{fig:mip_lobatto_c2_bar} are still non-smooth, though the largest spikes are removed.
...
Seeking a smoother curve for all values of $\Sigma_t \Delta x$ and $P$, we now examine the effect  using $Z_{MIP}=4$ in \figs{fig:mip_gauss_z4}{fig:mip_lobatto_z4}.
In \figs{fig:mip_gauss_z4}{fig:mip_lobatto_z4}, SL Gauss and SL Lobatto yield smoother curves for $\rho$ as a function of $\Sigma_t \Delta x$ for $P\in[1,4]$.  
However, there is an undesirable spike for SL Lobatto near $\Sigma_t \Delta x =100$.
As shown in \fig{fig:mip_lobatto_z4_bar}, averaging effectively eliminates the spike, suggesting that MIP DSA with $Z_{MIP}=4$ remains iteratively effective for SL Lobatto, for all $P$ for the case of spatially constant cross section.
%
%
%
%
