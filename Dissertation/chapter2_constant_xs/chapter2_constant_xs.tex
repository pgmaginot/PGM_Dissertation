...
neutron transport equations with pseudo- scattering, fission, and fixed sources.  
...
Additionally, we will assume that a steady-state neutron transport source-free pure absorber problem taxes DFEM schemes in a manner similar to the way DFEM schemes are tested
in time-dependent thermal radiative transfer simulations, in particular Marshak wave type problems \cite{ober_shadid}
...
Reed et. al \cite{reed} used arbitrary order DFEM $S_N$ transport in TRIPLET but, due to data storage limitations at the time, only LDFEM was computationally practical.
Possibly as a result of these historical computing limitations, the majority of reported DFEM radiation transport literature has focused on the LDFEM approximation.
...
Although not related to DFEM discretizations, we note that other approaches have been investigated to mitigate negativities; see, for instance, \cite{azmy}.
...
\section{Matrix Lumping Techniques to Study}
...
\begin{table}[!htp]
\centering
\caption{Nomenclature of numerical schemes (TL=Traditional Lumping; SL=Self-Lumping).}
\begin{tabular}{|c|c|c|} 
\hline
...
In Sec. \ref{sec:derivation}, we provide the weak form for the 1-D $S_N$ equations discretized with DFEM and define the different mass matrix lumping techniques.
...
\benum
\mu_d\frac{d \psi_d}{d x} + \sigma_t \psi_d = Q_d ,
\label{eq:slab_ex}
\eenum
...
In all that follows, we consider only non-scattering, non-fissioning media (pure absorbers), thus $Q_d$ will only be non-trivial if a fixed source i present in the problem.  
...
For $\mu_d>0$, $\psi_{in,d}$ is defined only at $x_{L}$ and  for $\mu_d<0$, $\psi_{in,d}$ is defined at $x_{R}$.  We begin our derivation by first transforming the physical geometry to a reference element, $s\in[-1,1]$. This affine transformation is such that:
\begin{subequations}
\label{eq:ref_map}
\beanum
x &=& \bar{x} + \frac{\Delta x}{2}s \\
dx &=& \frac{\Delta x}{2}ds
\eeanum
\end{subequations}
with $\bar{x}=\frac{x_{L} + x_{R}}{2}$ and $\Delta x = x_{R} - x_{L}$.  We seek a numerical approximation to the true angular flux $\psi_d$ using Lagrange polynomials of degree $P$:
\benum
\psi_d(s) \approx \widetilde{\psi}_d(s) = \sum_{j=1}^{N_P}{\psi_{j,d} \B{j}(s)} \pec
\label{eq:psi_rep}
\eenum
where the $\widetilde{\phantom{a}}$ denotes the numerical approximation.
...
\eeanum
\end{subequations}
In \eqt{eq:mu_in_p2} and \eqt{eq:mu_in_n}, $\psi_{in,d}$ is either the known angular flux outflow from the upwind cell or a boundary condition.  Inserting the definition of $\widetilde{\psi}_d(s)$, \eqt{eq:grad_term} becomes, for $\mu_d>0$,
\begin{multline}
...
\beanum
\mathbf{M}_{ij} &\approx& \sum_{q=1}^{N_q}{ w_q \B{i}(s_q) \B{j}(s_q) } \label{eq:mass_int}\\
\mathbf{G}_{ij} &\approx& sg(\mu_d)\B{i}(sg(\mu_d))\B{j}(sg(\mu_d)) - \sum_{q=1}^{N_q}{ w_q \frac{\p \B{i}}{\p s} \bigg \lvert_{s=s_q} \B{j}(s_q) }, %~~~\mu_d>0 \\
...
%
For instance, we consider $\delta$-shaped volumetric sources (i.e., equal to 0 everywhere except at one given point) as an example of a highly skewed volumetric source.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In Sec. \ref{sec:quad_select}, we discuss the properties of different numerical quadratures as applied to the 1-D DFEM $S_N$ equations.
...
\right. \pep
\eeanum
The highest polynomial degree a particular self-lumping quadrature formula exactly integrates is given in \tbl{tbl:int_acc} for Newton-Cotes, Gauss, and Lobatto quadratures.  
Also listed in \tbl{tbl:int_acc} is the maximum polynomial degree of the integrands present in the gradient and mass matrices.
\begin{small}
\begin{table}[!htp]
\centering
\caption{Accuracy of self-Lumping quadratures for trial spaces of different polynomial degree $P$ (each quadrature uses $N_P=P+1$ points).}
\begin{tabular}{|c|c|c|c|c|c|c|} 
\hline
  Polynomial 											& $N_P=$		  & Degree of   					 &  Degree of   					& Accuracy of &  Accuracy of  &  Accuracy of  \\
  Degree  of $\widetilde{\psi}$		& $P+1$		 		& $\mathbf{M}$ integrand & $\mathbf{G}$ integrand & Newton-Cotes &   Gauss 			&  Lobatto   \\
	\hline
				1   											&   2   		&   2   								&		1												&				1			&				3				&				1						
				\\ 		\hline
				2   											&   3   		&    4  								&			3											&				3			&				5				&				3					
				...
				3   											&   4   		&   6   								&			5 										&	3						&				7					&				5										\\		\hline
				4   											&   5   		&   8   								&			7 										&	5						&				9					&				7										\\		\hline
				5   											&   6   		&    10  								&			9											&	5						&				11				&				9										\\		\hline
				$P$  										 &   $P+1$   &   $2P$   							&	 $2P-1$									&  Odd $\widetilde{\psi}$: $P$  & $2P+1$		&		$2P - 1$						\\	
				{}											&							&											&															& Even $\widetilde{\psi}$: $P+1$ &			&			\\
					\hline
\end{tabular}
\label{tbl:int_acc} 
\end{table}
...
\end{enumerate}
The third point can easily be demonstrated. 
Indeed, with traditional lumping, $\mathbf{M}_{ij}$ is exactly computed and then a row-sum operation is performed on the rows of $\mathbf{ M}$; thus the entries of the diagonal mass matrix computed for TL are
\begin{align}
...
\begin{table}[!htp]
\centering
\caption{Equivalence of traditional lumping and Newton-Cotes approximation of the mass matrix.}
\begin{tabular}{|c|c|c|c|} 
\hline
  $P$			& Exact Integration of $\mathbf{M}$ 	&  Row Sum of $\mathbf{M}$ &  Newton-Cotes $w$\\
   {}			& 		{}									& 						{} 					&  with $P+1$ points \\
	\hline
				1   	&   
...
operator, as shown in \tbl{tbl:int_acc}.  
...
Thus, for linear and quadratic trial spaces, schemes that use (i) equally-spaced interpolation points and traditional lumping, or (ii) equally-spaced interpolation points and self-lumping numerical integration, or (iii) Lobatto quadrature points as interpolation points and self-lumping numerical integration will yield identical solutions.
...
For the first problem, we consider a source-free pure absorber with vacuum boundary conditions on the right, a known flux $\psi_{in,d}$ incident on the left face, and a spatially constant total cross section $\sigma_t$. 
...
\beanum 
\widetilde{\psi}_{in,d} &=& \sum_{j=1}^{N_P}{\psi_{j,d} \B{j}(-1) } \label{eq:cell_inflow} \\
...
%%\eenum
The analytic angular flux outflow, $\psi_{out,d} = \psi(x_R,\mu_d)$, is given by:
\benum
...
Figures \ref{fig:p1_outflow}-\ref{fig:p4_outflow} shows the numerically calculated cell outflow, $\widetilde{\psi}_{out,d}$, as a function 
of $h$ for all methods considered. 
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/P1_Outflow_AllMeth-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/P2_Outflow_AllMeth-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/P3_Outflow_AllMeth-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/P4_Outflow_AllMeth-eps-converted-to.pdf}
...
\end{itemize}
We also numerical verify the remarks made in Section \ref{sec:quad_select}, that is:
\begin{itemize}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P1_MFP_0.png}
\caption{Numerical inflow values as a function of $\frac{S_1}{S_0}$ for a single cell (vacuum case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:vac_inflow_p1}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P2_MFP_0.png}
\caption{Numerical inflow values as a function of $\frac{S_1}{S_0}$ for a single cell (vacuum case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:vac_inflow_p2}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P3_MFP_0.png}
\caption{Numerical inflow values as a function of $\frac{S_1}{S_0}$ for a single cell (vacuum case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:vac_inflow_p3}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P4_MFP_0.png}
\caption{Numerical inflow values as a function of $\frac{S_1}{S_0}$ for a single cell (vacuum case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P1_MFP_5.png}
\caption{Numerical inflow values  as a function of $\frac{S_1}{S_0}$, for a single cell (absorber case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:abs_inflow_p1}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P2_MFP_5.png}
\caption{Numerical inflow values  as a function of $\frac{S_1}{S_0}$, for a single cell (absorber case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:abs_inflow_p2}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P3_MFP_5.png}
\caption{Numerical inflow values  as a function of $\frac{S_1}{S_0}$, for a single cell (absorber case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:abs_inflow_p3}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Final_Inflow_RHS_Comparison_Source_P4_MFP_5.png}
\caption{Numerical inflow values  as a function of $\frac{S_1}{S_0}$, for a single cell (absorber case) with a $\delta$-shaped source, using polynomial orders $P=1$ through $P=4$.}
\label{fig:abs_inflow_p4}
\end{figure}
The total cell optical thickness was chosen to be 5 mean free paths because this value led to the clearest plots.
...
Regardless of trial space chosen,  all schemes exhibit some negativities, but the SL Gauss scheme exhibits the greatest negativities and oscillations. The SL Newton-Cotes scheme presents the least severe negativities.
...
\eeanum
\end{subequations}
%For $\mu_d > 0$, the numerical approximation of the angular flux near the cell inflow is given in \eqt{eq:cell_inflow} and the numerical approximation to the cell  outflow angular flux is given in \eqt{eq:cell_outflow}.
%Regardless of the sign of $\mu_d$, the numerical cell average angular flux, $\wideilde{\psi}_{A,d}$, for an arbitrary choice of interpolatory points,  is defined as
%\benum

%Note the small, but important distinction between $\widehat{\psi}_{in}$ and $\psi_{in}$ that arises due to the DFEM.  For $\mu>0$ $\widetilde{\psi}(x_L) \equiv \psi_{in}$,  whereas $\widehat{\psi}_{in} = \widetilde{\psi}(x\to x_{i-1/2})$.  That is to say, $\widehat{\psi}_{in} \neq \psi_{in}$ and $\widetilde{\psi}(x_L) \neq \psi_{in}$.  
%
%In \eqts{eq:num_defs}, $w_j$ is the weight associated with a given interpolation point, $s_j$, even if a particular numerical scheme is not self-lumping.  Solution representations with equally spaced interpolation points use $w_j$ corresponding to the $N_p$ point closed Newton-Cotes formula and Lobatto and Gauss interpolation points use the respective $N_p$ point Lobatto and Gauss quadrature weights.  This numerical integration to calculate $\widehat{\psi}_A$ is exact for any choice of interpolation point spacing and solution polynomial degree, since all quadratures can integrate polynomials of degree at least equal to $P$ and we are using $P+1$ interpolation points.
%The numerical approximations, $\widetilde{\psi}_{in,d}$, $\widetilde{\psi}_{A,d}$, and $\widetilde{\psi}_{out,d}$ are then expanded in a Taylor series of powers in $h$.
The Taylor expansions of the numerical analogues to the quantities in \eqts{eq:taylor_ex} 
%$\widetilde{\psi}_{in,d,T}$, $\widetilde{\psi}_{A,d,T}$, and $\widetilde{\psi}_{out,d,T}$, of the respective analytic Taylor expansions $\psi_{in,d,T}$, $\psi_{A,d,T}$, and $\psi_{out,d,T}$ 
depend on the trial space polynomial degree, the choice of interpolatory points, and the numerical integration strategy. For brevity, we omit
giving these numerical analogues.
\tbl{tbl:taylor_in}, \tbl{tbl:taylor_avg}, and \tbl{tbl:taylor_out} give the lowest order $h$ term and the magnitude of its constant (in parenthesis) of the difference between the exact Taylor-expanded quantities and the corresponding Taylor expansions of the numerical approximations to the angular flux near the cell inflow, cell average angular flux, and cell outflow angular flux, respectively.
\begin{landscape}
\begin{table}[!hbp]
\centering
...
\begin{table}[!htp]
\centering
\caption{Local truncation error analysis in $\widetilde{\psi}_{A,d}$ for a single cell problem with constant cross section.
Values given as $q(C)$ are to be read as $C h^q$ with $h=\sigma_t \Delta x / \mu$.  
``Machine Precision'' entries are meant to indicate that Taylor series analysis was inconclusive due to all coefficients being within machine precision.}
\begin{tabular}{|c|c|c|c|c|c|} 
\hline
  Polynomial 										  & Exact 											& TL  										& SL Newton-Cotes 					& SL Gauss 			 						& SL Lobatto  \\
  Degree  of $\widetilde{\psi}$		&   DFEM											& {}											& {}		 							 			& {}   											&	 {}   \\
...
\begin{table}[!htp]
\centering
\caption{Local truncation error analysis in $\widetilde{\psi}_{out,d}$ for a single cell with constant cross section.
Values given as $q(C)$ are to be read as $C h^q$ with $h=\sigma_t \Delta x / \mu$.
``Machine Precision'' entries are meant to indicate that Taylor series analysis was inconclusive due to all coefficients being within machine precision.}
\begin{tabular}{|c|c|c|c|c|c|} 
\hline
...
Here, we consider a homogeneous pure absorber material placed in a 1-D slab configuration and uniformly mesh the domain using $N_{cells}$ cells. We use: $x\in[0,10~cm]$, $\sigma_{t}=1~[cm^{-1}]$, no external sources, vacuum conditions on the right face of the slab, and a normally incident unit beam on the left face.
%\benum
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Linear_L2_err-eps-converted-to.pdf}
\caption{Convergence rate of the $L_2$ norm of the error, $E_{\psi}$,  as a function of the mesh cell size for a pure absorber ($\sigma_t(x) = 1~[cm^{-1}]$ and $x\in \left[0,10~[cm]\right]$).}
\label{fig:multi_L2_p1}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Quadratic_L2_err-eps-converted-to.pdf}
\caption{Convergence rate of the $L_2$ norm of the error, $E_{\psi}$,  as a function of the mesh cell size for a pure absorber ($\sigma_t(x) = 1~[cm^{-1}]$ and $x\in \left[0,10~[cm]\right]$).}
\label{fig:multi_L2_p2}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Cubic_L2_err-eps-converted-to.pdf}
\caption{Convergence rate of the $L_2$ norm of the error, $E_{\psi}$,  as a function of the mesh cell size for a pure absorber ($\sigma_t(x) = 1~[cm^{-1}]$ and $x\in \left[0,10~[cm]\right]$).}
\label{fig:multi_L2_p3}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Quartic_L2_err-eps-converted-to.pdf}
\caption{Convergence rate of the $L_2$ norm of the error, $E_{\psi}$,  as a function of the mesh cell size for a pure absorber ($\sigma_t(x) = 1~[cm^{-1}]$ and $x\in \left[0,10~[cm]\right]$).}
\label{fig:multi_L2_p4}
\end{figure}
Figures \ref{fig:multi_L2_p1}-\ref{fig:multi_L2_p4} mirror the results of \tbl{tbl:taylor_in}, which is expected since the convergence rate of $E_{\psi}$ will be limited by the slowest converging local approximation which is $\widetilde{\psi}_{in,d}$.  
Similarly, \figs{fig:multi_L2A_p1}{fig:multi_L2A_p4} are the multiple-cell analogue of the local truncation error analysis of $\widetilde{\psi}_{A,d}$ given in \tbl{tbl:taylor_avg}.  
\begin{figure}[!htp]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Linear_L2A_err-eps-converted-to.pdf}
...
\begin{figure}[!htp]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Quadratic_L2A_err-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Cubic_L2A_err-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Quartic_L2A_err-eps-converted-to.pdf}
...
\end{figure}
$E_{\psi_{out}}$, as shown in \figs{fig:multi_L2Out_p1}{fig:multi_L2Out_p4}, does not converge at the local truncation error rates of \tbl{tbl:taylor_out}.  
%
%
%
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Linear_L2Out_err-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Quadratic_L2Out_err-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Cubic_L2Out_err-eps-converted-to.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=12cm]{chapter2_constant_xs/Quartic_L2Out_err-eps-converted-to.pdf}
...
\end{figure}
The accumulation of errors in multiple-cell problems causes $E_{\psi_{out}}$ to globally converge one order of accuracy lower than the local truncation orders given in \tbl{tbl:taylor_out}. 
