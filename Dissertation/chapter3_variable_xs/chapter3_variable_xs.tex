...
\label{eq:slab_ex}
\eenum
In \eqt{eq:slab_ex}, $\psi_d(x)$ is the angular flux $\left[1 / [cm^2-sec-ster] \right]$ in $\mu_d$, the directional cosine relative to the $x$-axis, $\Sigma_t(x)$ is the total interaction cross section $[cm^{-1}]$, and $Q_d(x)$ is the total angular source in the direction of $\mu_d$ $\left[1 / [cm^3-sec-ster] \right]$. $Q_d$ includes both scattering and fixed sources.  
...
%\eenum
To determine the $N_P$ unknowns of \eqt{eq:psi_rep}, we follow the standard Galerkin procedure, successively multiplying \eqt{eq:slab_ex} by weight function $\B{i}$ and integrating the result by parts, hence generating $N_P$ moment equations. 
...
Using definition of \eqts{eq:mu_in_p}, \eqt{eq:grad_term} becomes, for $\mu_d>0$,
\begin{multline}
...
\end{multline}
%
For $\mu_d<0$, using the definition of \eqts{eq:mu_in_n} yields:
%
\begin{multline}
...
%
%
Considering all $N_P$ moment equations simultaneously, we write both \eqt{eq:weak_form_pos} and \eqt{eq:weak_form_neg} as:
\beanum
...
\label{eq:mat_form}
\eeanum
In \eqt{eq:mat_form} we use the following definitions for the vector of unknowns, $\psi_{j,d}$, and total source components, $Q_{j,d}$:
\beanum
...
In our testing of the EXS DFEM scheme, we use a 20-point Gauss-Legendre quadrature to approximately integrate \eqt{eq:react_mat}.  
Alternatively, adaptive quadrature, with a controllable tolerance, may be used such that the quadrature error in evaluating \eqt{eq:react_mat} could be reduced below some small tolerance.    
...
Schemes that are self-lumping evaluate the integrals of Eqs. (\ref{eq:react_mat})-(\ref{eq:stream_mat_neg}) using numerical quadrature.  By definition, self-lumping schemes create diagonal reaction and mass matrices:
\begin{subequations}
...
\eenum
with $\Sigma_t(x)$ defined as in \eqt{eq:xs_form}. 
...
%\end{array} \right] \pec
%\eenum
%which when used to solve \eqt{eq:mat_form} yields the following expression for angular flux outflow:
%\benum
%\label{eq:P3_NC_out}
...
%\end{multline}
%Obviously, there are combinations of $\Delta x$, $\Sigma_{t,2}$, and $\Sigma_{t,3}$ which can cause the numerator of \eqt{eq:P3_NC_out} to become negative, resulting in negative angular flux outflows.
...
%%  Arranging \eqt{eq:mfp_tot} $d_1$ will be defined as:
%%\benum
...
\label{eq:err_defs}
\end{subequations}
In \eqts{eq:err_defs}, $\Delta x_i$ is the width of cell $i$, cell $i$ spans $[x_{i-1/2},x_{i+1/2}]$, $\widetilde{\psi}_d(x)$ is the DFEM numerical approximation, $\psi(x,\mu_d)$ is the analytic solution (see \eqt{eq:exp_psi}).  The problem is spatially discretized using $N_{cells}$ spatial cells of equal width. 
...
\label{eq:sl_lump_inter}
\eenum

It must be emphasized that \eqt{eq:sl_lump_inter} is only used for plotting purposes.  
%For other purposes, such as calculating particle conservation, $L^2$ error estimates, etc., using the $\widetilde{IR}(s)$ given in \eqt{eq:sl_lump_inter} is either invalid or necessitates additional assumptions.
...
%\label{eq:conserve_cxs}
%\eenum
%%Results in a value of $\text{Total Absorption Rates}$ calculated by \eqt{eq:abs_fake} and \eqt{eq:conserve_cxs} are different.
%For \eqt{eq:abs_fake} and \eqt{eq:conserve_cxs} to be equivalent, $\Sigma_t(s)$ must be a constant, or $\hat{\Sigma}_{t,i}$ must calculated as in \eqt{eq:special}:
%\benum
%\label{eq:special}
%\hat{\Sigma}_{t,i} = \frac{\int_{-1}^1{ \Sigma_t(s) \widetilde{\psi}(s,\mu_d)~ds }}{\int_{-1}^1{\widetilde{\psi}(s,\mu_d)~ds}} \pep
%\eenum
%However, calculating $\hat{\Sigma}_{t,i}$ as in \eqt{eq:special} requires the transport solution to be known first, or requires a non-linear solution process.
...
\eenum
In \eqt{eq:ia_calc}, $\Sigma_t(s_j) = \hat{\Sigma}_t$ for the CXS DFEM scheme, and $\Sigma_t(s_j)$ is the point evaluation of the true cross section for all other schemes.
...
%Although less accurate than using a fine quadrature, we evaluate \eqt{eq:EIR} with quadrature restricted to the DFEM interpolation points, since the self-lumping schemes only conserve particles if $\widetilde{IR}(s)$ is evaluated at the angular flux DFEM interpolation points.
...
For our convergence study, we consider a source-free purely absorbing slab with a cross section that varies exponentially in space as in \eqt{eq:xs_form} with $c_1 = 0.1$ and $c_2 = 2\ln(10)$.  
...
CXS DFEM discretization being only a function of the cell optical thickness, which is preserved exactly by our definition of $\hat{\Sigma}_t$; see \eqt{eq:cxs_sigma}.
...
\label{eq:balance}
\eenum
As shown in \fig{fig:varxs_psi_out}, CXS DFEM converges the quantities on the left hand side of \eqt{eq:balance} with the same order of accuracy as any self-lumping scheme considered; CXS DFEM is at least as accurate in calculating the particles into a cell (outflow from the previous cell) and out of the cell (outflow from the current cell) as any other scheme considered.
Since \eqt{eq:balance} holds regardless of the numerical scheme considered, it follows that CXS DFEM converges $E_{IR_A}$, the term in the right hand side of \eqt{eq:balance} summed over all cells, with the maximum order of convergence displayed by any of the DFEM schemes we consider here.
...
In \fig{fig:cxs_blades}, we plot the exact $\psi(x)$ and $IR(x)$, as well as their CXS DFEM numerical approximations, $\widetilde{\psi}(x)$ and $\widetilde{IR}(x)$, using $N_{cells}=5$, and $\hat{\Sigma}_{t,i}$ as defined in \eqt{eq:cxs_sigma}. 
...
Finally, we consider the $\widetilde{\psi}_d(x)$ and $\widetilde{IR}(x)$ solution representations of the FW CXS scheme.  In \fig{fig:fw_blading} it is clear that while the FW CXS and CXS DFEM schemes calculate slightly different solution representations, the FW CXS scheme  exhibits the same interaction rate blading phenomena as the CXS DFEM scheme, reiterating that blading is a result of approximating a spatially varying cross section with as a cell-wise constant.
...
With the MFP meshing strategy, we find each cell width by determining the width of each cell from $i=1$ (leftmost cell) to $i=N_{cell}$ as outlined by \eqt{eq:mfp_spacing}:
\begin{subequations}
...
\eenum
\end{subequations}
%PSI spacing requires a priori knowledge of the analytic solution.  For the pure absorber problem, it is thus feasible to use PSI spacing, but in general PSI spacing would not be feasible.  Starting again from $i=1$, we find the width of each cell through the logic outlined in \eqt{eq:psi_spacing}
%\begin{subequations}
%\label{eq:psi_spacing}
%\beanum
