...
Historically, the neutron transport and thermal radiative transfer communities assumed interaction cross section and opacities, respectively, that were cell-wise constant \cite{lewis_book,morel_radtran,adams}.
...
Our work differs from \cite{adams_scb,adams_nowak,varXS_diff,varXS_MOC} by considering a discontinuous finite element (DFEM) spatial discretization of the slab geometry $S_N$ transport equation using arbitrary degree polynomial finite element trial spaces.  
...
We build on the quadrature integration ideas presented in \secref{sec:chapter2_constant_xs} and employ a numerical quadrature to evaluate the mass matrix integrals that involve cross sections as a function of space.
...
We begin by repeating the DFEM neutron transport equation derived in \secref{sec:chapter2_constant_xs}:
\benum
...
\left( \mu_d \mathbf{G} + \mathbf{R}_{\Sigma_t} \right) \vec{\psi}_d = \vec{Q}_d + \mu_d \psi_{in,d} \vec{f} \pep
\eenum
The $N_P \times N_P$ reaction matrix, $\mathbf{R}_{\Sigma_t}$ is defined as:
\benum
\mathbf{R}_{\Sigma_t,ij} = \frac{\Delta x}{2}\int_{-1}^1{\Sigma_t(s)\B{i}(s)\B{j}(s)~ds} \pep
...
\benum
\mathbf{R}_{\Sigma_t,ij} = \frac{\Delta \Sigma_t}{2} \int_{-1}^1{\B{i}(s)\B{j}(s)~ds} \pec
\eenum
...
A shorthand notation is given in \tbl{tbl:names_chap3} for all of the numerical methods considered in this section and described in detail in the remainder of this section.
\begin{table}[!htp]
\centering
...
Historically, neutronics and some radiative transfer calculations have approximated spatially varying cross sections by assuming cell-wise constant cross sections \cite{lewis_book, morel_radtran, adams, warsa_krylov}.  
...
Schemes that are self-lumping evaluate the integrals of \eqt{eq:chap3_react_mat} using numerical quadrature.  
In \secref{sec:chapter2_constant_xs} we showed that by definition, self-lumping schemes create diagonal mass matrices.
...
w_i \frac{\Delta x}{2} \Sigma_t(s_i) ~ & ~ i=j \\
 0 & ~\text{otherwise}
\end{array}
\right. \\
\eenum
Though the choice of interpolation points does not affect exact integration schemes, as shown in \secref{sec:chapter2_constant_xs}, the choice of interpolation points was shown to influence both the robustness and accuracy of self-lumping schemes.  
...
\caption{Numerical outflow from single cell pure absorber with $\Sigma_t(x) = c_1e^{c_2 x}$, as a function of $c_2$ with constant optical thickness for linear DFEM.}
...
\caption{Numerical outflow from single cell pure absorber with $\Sigma_t(x) = c_1e^{c_2 x}$, as a function of $c_2$ with constant optical thickness for quadratic DFEM.}
...
\caption{Numerical outflow from single cell pure absorber with $\Sigma_t(x) = c_1e^{c_2 x}$, as a function of $c_2$ with constant optical thickness for cubic DFEM.}
...
\caption{Numerical outflow from single cell pure absorber with $\Sigma_t(x) = c_1e^{c_2 x}$, as a function of $c_2$ with constant optical thickness for quartic DFEM.}
...
\caption{Convergence of $E_{\psi}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with linear DFEM.}
...
\caption{Convergence of $E_{\psi}$ as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quadratic DFEM.}
...
\caption{Convergence of $E_{\psi}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with cubic DFEM.}
...
\caption{Convergence of $E_{\psi}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quartic DFEM.}
...
\caption{Convergence of $E_{\psi_A}$   as a function of mesh size for a pure absorber with exponentially varying cross section discretized with linear DFEM.}
...
\caption{Convergence of $E_{\psi_A}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quadratic DFEM.}
...
\caption{Convergence of $E_{\psi_A}$   as a function of mesh size for a pure absorber with exponentially varying cross section discretized with cubic DFEM.}
...
\caption{Convergence of $E_{\psi_A}$   as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quartic DFEM.}
...
\caption{Convergence of $E_{\psi_{out}}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with linear DFEM.}
...
\caption{Convergence of $E_{\psi_{out}}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quadratic DFEM.}
...
\caption{Convergence of $E_{\psi_{out}}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with cubic DFEM.}
...
\caption{Convergence of $E_{\psi_{out}}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quartic DFEM.}
...
\caption{Convergence of $E_{IR}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with linear DFEM.}
...
\caption{Convergence of $E_{IR}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quadratic DFEM.}
...
\caption{Convergence of $E_{IR}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with cubic DFEM.}
...
\caption{Convergence of $E_{IR}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quartic DFEM.}
...
\caption{Convergence of $E_{IR_A}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with linear DFEM.}
...
\caption{Convergence of $E_{IR_A}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quadratic DFEM.}
...
\caption{Convergence of $E_{IR_A}$ as a function of mesh size for a pure absorber with exponentially varying cross section discretized with cubic DFEM.}
\label{fig:varxs_I_A_p3}
\end{figure}
\begin{figure}[!hbp]
\centering
\includegraphics[width=10cm]{chapter3_variable_xs/P4_VarXS_E_I_A.png}
\caption{Convergence of $E_{IR_A}$  as a function of mesh size for a pure absorber with exponentially varying cross section discretized with quartic DFEM.}
...
\caption{Plots of the analytic $\psi(x)$, CXS DFEM $\widetilde{\psi}(x)$, and cell-wise constant cross section analytic $\psi_C(x)$, for the pure absorber with exponential cross section.}
...
\caption{Plots of the analytic $IR(x)$, CXS DFEM $\widetilde{IR}(x)$, and  cell-wise constant cross section analytic $IR_C(x)$ for the pure absorber with exponential cross section.}
...
Consider $\widetilde{\psi}(x)$ and $\widetilde{IR}(x)$ obtained with SLXS Lobatto using a linear DFEM trial space and five spatial cells, shown in \fig{fig:lobatto_blades_psi} and \fig{fig:lobatto_blades_ir}, for the same problem.  
...
\caption{$E_{\psi}$ for FW CXS vs. CXS DFEM for a pure absorber with exponentially varying cross section using a cubic DFEM trial space.}
...
\caption{$E_{\psi_A}$ for FW CXS vs. CXS DFEM for a pure absorber with exponentially varying cross section using a cubic DFEM trial space.}
...
\caption{$E_{IR}$ for FW CXS vs. CXS DFEM for a pure absorber with exponentially varying cross section using a cubic DFEM trial space.}
...
\caption{$E_{IR_A}$ for FW CXS vs. CXS DFEM for a pure absorber with exponentially varying cross section using a cubic DFEM trial space.}
...
For the pure absorber problem, we compare two alternative methods of mesh spacing (logarithmic grids and equal optical thickness grids, see \tbl{tbl:spacing_labels}), to the results obtained with equally-spaced mesh cells.
...
\caption{Asymptotic convergence of $E_{\psi}$ for the SLXS Lobatto scheme using a quadratic trial space with different mesh spacing methodologies.}
...
\caption{Asymptotic convergence of $E_{\psi_A}$ for the SLXS Lobatto scheme using a quadratic trial space with different mesh spacing methodologies.}
...
\caption{Asymptotic convergence of $E_{IR}$ for the SLXS Lobatto scheme using a quadratic trial space with different mesh spacing methodologies.}
...
\caption{Asymptotic convergence of $E_{IR_A}$ for the SLXS Lobatto scheme using a quadratic trial space with different mesh spacing methodologies.}
...
\caption{$E_{\psi}$ for SLXS Lobatto using a quadratic trial space scheme with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{\psi_A}$ for SLXS Lobatto using a quadratic trial space scheme with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{IR}$ for SLXS Lobatto using a quadratic trial space scheme with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{IR_A}$ for SLXS Lobatto using a quadratic trial space scheme with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{\psi}$ for SLXS Gauss using a quadratic trial space with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{\psi_A}$ for SLXS Gauss using a quadratic trial space with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{IR}$ for SLXS Gauss using a quadratic trial space with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{IR_A}$ fore SLXS Gauss using a quadratic trial space with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{\psi}$ for CXS DFEM using a quadratic trial space  with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{\psi_A}$ for CXS DFEM using a quadratic trial space  with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{IR}$ for CXS DFEM using a quadratic trial space  with different mesh spacing methodologies, at low resolutions.}
...
\caption{$E_{IR_A}$ for CXS DFEM using a quadratic trial space  with different mesh spacing methodologies, at low resolutions.}
...
\section{Conclusions to Be Carried Forward}
