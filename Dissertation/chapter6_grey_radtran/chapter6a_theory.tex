...
\beanum
g(0) &=& g_0 \\
\frac{\partial g}{\partial t}&=& f(t,g) \pec
\eeanum
...
\benum
g_i = g_{n} + \Delta t \sum_{j=1}^i{a_{ij} f\left( t_j , g_j\right)} \pec
\label{eq:psi-def}
\eenum
where $g_i$ is the intermediate value of $g$ at the time of stage $i$ and $t_j = t^n + \Delta t c_j$.
...
\label{eq:k_T}
\eenum
We evaluate $k_{I,s}$ and $k_{T,s}$, the SDIRK $k$ values (\eqt{eq:p2}), for intensity and temperature for stage $s$ as:
\benum
...
\begin{subequations}
\label{eq:xs_analytic_pseudo}
\beanum
\widetilde{\Sigma_t} &=& \sigma_{\tau,1} \\
\widetilde{\Sigma_s} &=& \sigma_s + \nu_1 \sigma_a\pep
\eeanum
\end{subequations}
By definition, $\widetilde{\Sigma_s} < \widetilde{\Sigma_t}$. 
This can be seen by considering the definition of \eqt{eq:analytic_nu_def}, and realizing that $\nu_1 < 1$.
Since $\widetilde{\Sigma_t} > \sigma_t$, and $\widetilde{\Sigma_s} < \sigma_t$, it follows that $\widetilde{\Sigma_s} < \widetilde{\Sigma_t}$.
Inserting \eqts{eq:xs_analytic_pseudo} into \eqt{eq:i_analytic_pseudo} yields
\benum
...
\benum
I_i = I_n + \highlight{ \Delta t \sum_{j=1}^{i-1}{a_{ij} k_{I,j}} } + a_{ii} \Delta t c \left[ \frac{1}{4\pi}\sigma_{s} \phi_i + \sigma_a \left[B_* + D_* (T_i - T_*) \right]+ S_I - \mu_d \frac{\partial I_i}{\partial x} - \sigma_t I_i \right] 
\pep
\label{eq:i_i_start}
\eenum
Likewise for $T_i$, we have
\benum
T_i = T_n + \highlight{ \Delta t \sum_{j=1}^{i-1}{a_{ij} k_{T,j}} } + \frac{a_{ii} \Delta t }{C_v} \left[ \sigma_a \left( \phi_i - 4\pi \left[ B_* + D_*(T_i - T_*) \right] \right) + S_T  \right] \pep
\label{eq:analytic_t_i_start}
\eenum
The only difference between \eqt{eq:i_i_start} and \eqt{eq:long_i_1_analytic} is the addition of
\benum
\Delta t \sum_{j=1}^{i-1}{a_{ij} k_{I,j}} \pep
\eenum
Likewise, the only difference between \eqt{eq:analytic_t_i_start} and \eqt{eq:long_t_1} is the additional term
\benum
...
\left( T_n - T_* + \highlight{ \Delta t \sum_{j=1}^{i-1}{a_{ij} k_{T,j}} }+  \frac{a_{ii} \Delta t }{C_v} \left[ \sigma_a \left( \phi_i - 4\pi   B_* \right) + S_T \right]  \right) \pec
\label{eq:analytic_t_i}
\end{multline}
where we have highlight that the stage $i$ SDIRK temperature equation differs from the stage 1 SDIRK temperature update equation, \eqt{eq:analytic_t_1}, by only an additional term.
This result is significant because \eqt{eq:analytic_t_1}  could be thought of as being unique to the implicit Euler discretization which is only first order accurate in time.
Modifying \eqt{eq:analytic_t_1} by the addition of only a single term, or alternatively, by thinking in terms of arbitrary stage count SDIRK time integration schemes rather than deriving the thermal radiative transfer equations for a single time discretization (implicit Euler), allows for arbitrary order accuracy time integration.
...
\begin{subequations}
\label{eq:analytic_pseudo_i_defs}
\benum
\nu_i = 4\pi \sigma_a D_* \left(1 + \frac{4\pi a_{ii} \Delta t}{C_v} \sigma_a D_*  \right)^{-1}  \frac{a_{ii} \Delta t }{C_v} = \frac{ 4\pi a_{ii} \Delta t \sigma_a D_*}{C_v + 4\pi a_{ii} \Delta t  \sigma_a D_*} \pec
\label{eq:analytic_nu}
\eenum
...
\eenum
\begin{multline}
\highlight{ \xi_i } = \sigma_a B_* + S_I +  \frac{1}{a_{ii} \Delta t c} I_n + \frac{1}{a_{ii} c} \sum_{j=1}^{i-1}{a_{ij} k_{I,j}}  + \dots  \\ 
\sigma_a D_*\left(1 + \frac{4\pi a_{ii} \Delta t}{C_v} \sigma_a D_*  \right)^{-1} 
\left( T_n - T_* + \highlight{ \Delta t \sum_{j=1}^{i-1}{a_{ij} k_{T,j}} } +   \frac{a_{ii} \Delta t }{C_v}  \left( S_T -  4\pi  \sigma_a B_*   \right) \right) \pep
\end{multline}
\end{subequations}
Again, we have highlighted in \eqts{eq:analytic_pseudo_i_defs} the only structural changes that result by considering arbitrary order accuracy SDIRK time integration of the thermal radiative transfer equations instead of limiting oneself to implicit Euler time integration of the thermal radiative transfer equations.
...
\noindent On the basis that:
\vspace{0.25in}
\\
\emph{
\noindent a negative intensity contributes to a negative time derivative of the temperature, the
Planck function at negative temperatures should similarly contribute to a negative time derivative of the intensity.
This implies that the Planck function at negative temperatures should be negative, 
}
\vspace{0.25in}

\noindent Morel et al. \cite{negative_trt} argued that for an arbitrary positive temperature $\bar{T}$,
\beanum 
B(-\bar{T}) &=& -B(\bar{T}) \\
\frac{dB}{dT} \bigg \lvert_{T = -\bar{T} } &=& -\frac{dB}{dT} \bigg \lvert_{T =\bar{T} } \pep
\eeanum
...
Additionally, after a time step, we would like to verify that the solution did not change too rapidly over a single time step.
We achieve these goals by using adaptive time stepping algorithms.
...
As in \cite{time_adaptive_diffusion}, given the current time step, $\Delta t^n$, $\Delta t^n = t^n - t^{n-1}$, the next time step, $\Delta t^{n+1}$, $\Delta t^{n+1} = t^{n+1} - t^n$, is predicted as:
\benum
\Delta t^{n+1} = \frac{\Delta T_{goal} }{\Delta T} \Delta t^n  \pec
\label{eq:adaptive_change}
\eenum
where $\Delta T_{goal}$ and $C_{max}$ are user prescribed values and $\Delta T$ is a measure of the change in temperature that occurred over $\Delta t^n$.  
...
\label{eq:min_dt}
\eenum
We use the adaptive criterion to verify that in advancing the solution from $t^{n-1}$ to $t^n$ the solution has not changed too rapidly by enforcing
\benum
...
In \cite{time_adaptive_diffusion}, a point-wise  formula is prescribed to calculate the adaptive time criterion $\Delta T$, 
\benum
\Delta T = 2 \max_{i=1,\dots,N_{cell}} \left[  \max_{j=1,\dots N_P} \left[ \frac{\abs{T_{j}^{n} - T_{j}^{n-1}} }{ T_{j}^{n} + T_{j}^{n-1} }   \right] \right] \pep 
\label{eq:pointwise_adaptive}
\eenum
If calculating $\Delta T$ according to \eqt{eq:pointwise_adaptive}, we say that we are using the \emph{point-wise} adaptive criterion.
...
To account for the possibility of lower, possibly even negative temperatures, we also consider a \emph{modified point-wise} temperature criterion:
\benum
\Delta T = 2 \max_{i=1,\dots,N_{cell}} \left[  \max_{j=1,\dots N_P} \left[ \frac{\abs{T^{n+1}_j- T^{n}_j } }{ \max\left(\abs{ T^{n+1}_j } + \abs{ T^n_j } , T_{offset}  \right)  }\right]  \right] \pec
\label{eq:modified_pointwise_adaptive}
\eenum
where $T_{offset}$ is a user selected temperature.  
In practice, \eqt{eq:modified_pointwise_adaptive} with $T_{offset} = 0$ works for problems in where \eqt{eq:pointwise_adaptive} fails,  but still chooses very small time steps.
...
We refer to our final time adaptive criterion as the  \emph{volumetric} adaptive criterion.
The volumetric adaptive criterion is inspired by \eqt{eq:pointwise_adaptive}.
Dividing the spatial mesh into $N_{groups}$ contiguous groupings of cells, each grouping with $N_{cg}$ cells, such that $N_{cell} = N_{groups} \times N_{cg}$, the volumetric adaptive criterion defines $\Delta T$ as:
\benum
...
\benum
\norm{ \widetilde{T}  }_{g,2}= \sqrt{ \int_{x_{g-1/2}}^{x_{g+1/2}}{ \left( \widetilde{T} \right)^2 ~dx }  }\pec
...
When $N_{cg} = 1$, the volumetric adaptive criterion is most like the point-wise adaptive criteria.  
However, increasing $N_{cg}$ has the same effect as increasing $T_{offset}$ in \eqt{eq:modified_pointwise_adaptive}, thus we will investigate the effect of $N_{cg}$ on the time step sizes chosen by the volumetric adaptive criterion.
