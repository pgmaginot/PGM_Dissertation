...
for m=1:1:3
    ...
    for p=1:1:4
        ...
        [dfem_mat,junk] = feshpln( linspace(-1,1,p +3) , quad_pt ,p);
        
        cell_base = {   sprintf('Marshak_Results/%s_P%i_20_cell',cell_meth,p) ...
                        sprintf('Marshak_Results/%s_P%i_80_cell',cell_meth,p) ...
                        sprintf('Marshak_Results/%s_P%i_320_cell',cell_meth,p) ...
                        sprintf('Marshak_Results/%s_P%i_1280_cell',cell_meth,p) };
        ...
        if(m==3)
            ...
            data_base = {   sprintf('Marshak_Results/Marshak_Wave_%s_P%i',data_meth,p) ...
                            sprintf('Marshak_Results/Marshak_Wave_%s_P%i_Refine_4_Cells_64_Time',data_meth,p) ...
                            sprintf('Marshak_Results/Marshak_Wave_%s_P%i_Refine_16_Cells_64_Time',data_meth,p) };
                       
        else
            data_base = {   sprintf('Marshak_Results/Marshak_Wave_%s_P%i_Base',data_meth,p) ...
                            ...
                            sprintf('Marshak_Results/Marshak_Wave_%s_P%i_Base_Refine_64_Cells_64_Time',data_meth,p) };
        end
        Dark_Arts_Plotter((m-1)*4 + p,10 + (m-1)*4 + p ,cell_base,data_base,dfem_mat)
        
        temperature_file = sprintf('Marshak_Zoom_Temperature_%s_P%i_Cell_Refinement',data_meth,p);
        rad_file = sprintf('Marshak_Zoom_Radiation_%s_P%i_Cell_Refinement',data_meth,p);
...
set(h,'Interpreter','latex','FontSize',14);
SavePretty( figure(9) , 'Blading_Temperature_Full_MultiCell');

axis([0.3 0.42 -0.05 1.1])
SavePretty( figure(9) , 'Blading_Temperature_Zoom_MultiCell');

figure(19)
axis([0 1 -0.1 2.2])
h = legend('20 Cell' , '80 Cell' , '320 Cell' );
set(h,'Interpreter','latex','FontSize',14);
SavePretty( figure(19) , 'Blading_Radiation_Full_MultiCell');