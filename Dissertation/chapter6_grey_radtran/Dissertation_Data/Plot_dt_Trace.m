function Plot_dt_Trace(iter_files,max_steps)
    ...
    for i=1:1:n_files
        ...
        [n_steps, n_repeat, dt_accepted] = Parse_Full_Iteration_File(fid, max_steps(i,1) );
        fprintf('%s :  repeats %i  n_steps %i \n',char( iter_files(i,1) ) , n_repeat , n_steps);

        ts = 1:1:n_steps;
        ts  = ts ./ n_steps;
        hold on
        plot(ts,dt_accepted, char( color_str(1,i) ) , 'Color' , color_str{2,i} ,'LineWidth',1.5)
        fclose(fid);
    end

    set(gca,'YScale','log')
    axis([0 1 1E-9 1E-3])
    xlabel('$\frac{n}{n_{final}}$', 'Interpreter','latex','FontName','Times','FontSize',24)
