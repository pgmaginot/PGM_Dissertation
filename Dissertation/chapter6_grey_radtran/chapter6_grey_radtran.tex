...
\R{\sigma_a*}\left(\vec{\phi}_i - 4\pi\vec{B}_* - 4\pi\D\left( \vec{T}_i - \vec{T}^* \right)\right) + \vec{S}_T 
\right]
\label{eq:first_kTi}
\eenum
%
...
%
\be
\vec{T}_i +4\pi\Delta t a_{ii}\R{C_v}^{-1}\R{\sigma_a*}\D\vec{T}_i = \vec{T}_n + \Delta t \sum_{j=1}^{i-1}{a_{ij} k_{T,j}   } + \Delta t a_{ii}
\R{C_v}^{-1}\left[
...
%
\be
\left[\I + 4\pi\Delta t a_{ii}\R{C_v}^{-1}\R{\sigma_a}\D  \right]\vec{T}_i = \vec{T}_n + \Delta t \sum_{j=1}^{i-1}{a_{ij} k_{T,j}   } + \Delta t a_{ii}\R{C_v}^{-1}\left[\R{\sigma_a*} \left(\vec{\phi}_i - 4\pi\vec{B}_* + 4\pi\D\vec{T}^* \right) + \vec{S}_T \right]
\ee
%
%
\begin{multline*}
...
\benum
\mathbf{ L} \vec{I}_i + \bar{\bar{\mathbf R}}_{\sigma_t,i}\vec{I}_i = \frac{1}{4\pi}\R{\sigma_s,*}\vec{\phi}_i + \frac{1}{4\pi}\bar{\bar{\mathbf \nu}}_i \R{\sigma_a*}\vec{\phi}_i + \bar{\bar{\mathbf \xi}}_{i,d} + \vec{f}I_{in} 
\eenum
