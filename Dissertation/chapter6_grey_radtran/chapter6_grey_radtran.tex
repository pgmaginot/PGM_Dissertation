...
The apparent gaps in \fig{fig:sig_a_1000_radiation} and \fig{fig:sig_a_1000_temperature} are discontinuities in the solution representation at cell edges.
Though \fig{fig:sig_a_1000_radiation} and \fig{fig:sig_a_1000_temperature} use 1000 spatial cells, the 1000 spatial cells are equally distributed for $x\in[0,1]$, whereas, thermal wave travels only to $x<0.15$.
...
We now consider a Marshak wave problem that uses physical units.
...
The apparent discontinuities and overshoots in \figs{fig:physical_slices_radiation}{fig:physical_slices_temperature} are the tails of the cubic finite element in some mesh cells.
...
Time step size selection as function of the simulated time is given for each method in \fig{fig:linear_time_steps}.
\begin{figure}[!htp]
\centering
...
The point-wise and modified point-wise schemes choose larger $\Delta t$ when the thermal wave is in the middle of a mesh cell.
...
\end{tabular}
\end{table}
The point-wise and modified point-wise results in \tbl{tbl:p1_steps} are comparable to the results reported in \cite{time_adaptive_diffusion}, where the point-wise adaptive scheme was applied to a 50 cell linear DFEM radiative diffusion discretization, reported to take about 33,000 time steps when using $\Delta T_{goal} = 0.01$.
...
that negative temperature solutions cause the simplest point-wise adaptive criterion, \eqt{eq:pointwise_adaptive}, to fail.
...
To show that the it is only the challenging nature of an under resolved Marshak wave problem causes the point-wise adaptive criterion to fail,  we consider an alternative to the physical Marshak wave problem.
...
Comparing the number of steps taken in \tbl{tbl:p3_counts} to the number of steps taken in \tbl{tbl:p1_steps} makes it clear that higher order DFEM methods applied to TRT simulations challenge the performance of simple time adaptive criteria.
...
This can be seen more clearly by considering a zoom in if time step selection, as given in \fig{fig:mod_comparison_zoom}.
\begin{figure}[!htp]
\centering
...
The increased numbers of peaks and valleys in \fig{fig:mod_comparison} as compared to \fig{fig:linear_time_steps} is a result of increased spatial cell count and moving from linear to cubic DFEM.
Consider \fig{fig:p1_50c_vs_100c} that plots the time step trace of linear SLXS Lobatto simulations using 50 and 100 spatial cells for $S_8$ angular differencing with the point-wise adaptive criteria.
\begin{figure}[!htp]
\centering
...
\end{figure}
At early times, such as those considered in \fig{fig:p1_50c_vs_100c}, the accumulation of temporal errors is minimal, and despite the different spatial resolutions affecting wavefront location, for our purposes here, we assume the thermal waves are at the same position in the simulation. 
...
Though the volumetric adaptive criterion with $N_{cg}=1$  requires more than twice as many time steps to solve the same Marshak wave problem with cubic rather than linear DFEM, the modified point-wise scheme requires more than $7\times$ time steps to solve the same problem with cubic rather than linear DFEM. 
...
Consider now the trace of $\Delta t$ selection as a function of time in the simulation in \fig{fig:5ev_1C} for the modified point-wise scheme with $T_{offset}=5~[eV]$ to the volumetric adaptive criterion with $N_{cg}=1$.
\begin{figure}[!htp]
\centering
...
The dips we have thus far seen and associated with the thermal wave crossing individual mesh cells are present, but in \fig{fig:5c_vs_25c}, there are additional lower frequency dips in $\Delta t$.
...
Even when focusing in on the Marshak wavefront, as in \fig{fig:time_difference_radiation_zoom} and \fig{fig:time_difference_temperature_zoom}, 
%
%
%
\begin{figure}[!htp]
...
In \tbl{tbl:iteration_count}, we give a sampling of the average number of iterations required to update the intensity for a given thermal iteration for different MMS and unity Marshak wave simulations.
Iteration counts for a designed optically thick and diffusive problem are given in \tbl{tbl:high_iter_count}.
...
To demonstrate the iterative effectiveness of MIP DSA we now present a problem designed solely to be optically thick and diffusive.
...
\benum
\frac{ \norm{ \phi^{(\ell+1) } - \phi^{(\ell)} } }{ \norm{\phi^{(\ell+1)}}} < \epsilon_{\phi} \pec
\label{eq:no_norm}
\eenum
can lead to false convergence \cite{adams_larsen_fast_iterative}.
Noting that our chosen point-wise convergence condition $\text{change\_phi} < \epsilon_{\phi}$, repeated here from \eqt{eq:change_phi}
\benum
...
To do so, we use the $L^1$ norm variant of \eqt{eq:no_norm}:
\benum
\frac{ \norm{ \phi^{(\ell+1)} - \phi^{(\ell)} }_{L^1} }{  \norm{ \phi^{(\ell+1)}}_{L^1} } < \epsilon_{\phi} \pep
\label{eq:l1}
\eenum
...
\benum
\frac{ \norm{\phi^{(\ell+1)} - \phi^{(\ell)} }_{L^1} }{ (1- \rho) \norm{\phi^{(\ell+1)} }_{L^1} }< \epsilon_{\phi} \pec
\label{eq:rho_convergence}
\eenum
where $\rho$ is the spectral radius, which we estimate as:
\benum
\rho \approx \frac{ \norm{\phi^{(\ell+1)} - \phi^{(\ell)} }_{L^1} }{ \norm{\phi^{(\ell)} - \phi^{(\ell-1)} }_{L^1} }\pep
\eenum
In \tbl{tbl:rho_iters}, we plot the average number of iterations required to converge $\phi$ for each time step for our optically thick and diffusive test problem.  
