...
We now attempt to determine if \R{\widetilde{\Sigma}_s} is equivalent to 
\benum
\R{\sigma_s} + \overline{\overline{\nu}}_i \R{\sigma_a} \pep
\eenum
First, we simply expand the definition of $\R{\widetilde{\Sigma}_s}$ from \eqts{eq:funky_defs}, to incorporate the defintion of $\nu_i$ given in \eqt{eq:analytic_nu},
\benum
...
Given this information, in the most general case, when using MIP DSA to accelerate grey radiative transfer, we will define the integration in cell $c$ (\eqt{eq:mip_sigma_a} ) as: 
\benum
( \Sigma_a \Delta \phi, \B{*} ) = \R{\sigma_{\tau,i}} - \left( \R{\sigma_s} + \overline{\overline{\nu}}_i \R{\Sigma_a} \right) \vec{\Delta \phi}_c \pec
\eenum
...
Regardless of whether we use exact matrix construction, traditional lumping, or self lumping integration, the our solution of the radiative transfer equations requires access to a mass matrix, for example when calculating $\vec{k}_I$ in \eqt{eq:k_i_vec_example}.
...
The method of manufactured solutions (MMS)\cite{mms} consists of choosing an analytic solution, then defining the driving source necessary to achieve that solution.
...
Figure \ref{fig:mms1_tl_phi}, \fig{fig:mms1_lobatto_phi}, and \fig{fig:mms1_gauss_phi} respectively plot the order of convergence of $E_{\phi}$ for the TL, SL Lobatto, and SL Gauss schemes, respectively as a function of trial space degree and cell size.
...
First, the TL scheme does not necessarily increase in accuracy with an increase in trial space degree, is limited to second order spatial convergence for odd degree trial space DFEM, and is limited to their order spatial convergence for even degree trial space schemes.
...
However, comparing \fig{fig:mms1_gauss_phi_A} to \figs{fig:multi_L2A_p1}{fig:multi_L2A_p4}, it appears that SL Gauss applied to the radiative transfer problems loses at most 1 order of convergence relative to SL Gauss applied to neutron transport, whereas SL Lobatto loses at least one order of convergence.
Interestingly, decrease in convergence of $E_{\phi_A}$ does not translate to $E_{T_A}$, SL Lobatto converges $E_{T_A}$ with the same order as $E_{\psi_A}$ and $E_{IR_A}$ are converged by SL Lobatto for neutron transport, and SL Gauss actually exhibits an $E_{T_A}$ order of convergence that exceeds its neutron transport analogs. 
\begin{figure}[!ht]
\centering
...
Secondly, as the TRT equations are functions of space and time, it possible that the error of a given spatial discretization is smaller than the temporal error of the problem, thus a plateauing of the solution error as a function of space.
...
The methods denoted SL will assume cell wise constant opacities and specific heats,  equal to the volumetric average of that quantity.  
...
Confirming our suspicion that convergence of $E_T$ is limited, we plot the convergence of $E_T$ for the SL Lobatto scheme in \fig{fig:mms3_constant_lobatto_temp} and for the SL Gauss scheme in \fig{fig:mms3_constant_gauss_temp}.
\begin{figure}[!htp]
\centering
...
Moving on to the convergence of $E_T$, in \fig{fig:mms3_slxs_lobatto_e_t} observe that SLXS Lobatto converges $E_T \propto P$, the same rate SL Lobatto converged $E_T$ for the TRT problem with constant material properties.  
In \fig{fig:mms3_slxs_gauss_e_t} converges very rapidly, and only the asymptotic convergence rate of linear and quadratic DFEM can be estimated, though both suggest SLXS Gauss converges $E_T \propto +P+2$.
\begin{figure}[!htp]
\centering
...
For completeness, we include convergence plots of $E_{T_A}$ for SLXS Lobatto and SLXS Gauss respectively in \fig{fig:mms3_slxs_lobatto_t_a} and \fig{fig:mms3_slxs_gauss_t_a}, however, all we can conclude is that both SLXS Lobatto and SLXS Gauss experience increases in convergence of $E_{T_A}$ with increases in trial space degree.
\begin{figure}[!htp]
\centering
...
Though very similar to MMS2, we hope that by considering a steady state problem, can study the spatial error of our DFEM schemes without temporal error interference.
...
\begin{figure}[!hbp]
\centering
\includegraphics[width=10cm]{chapter6_grey_radtran/Dissertation_Data/Constant_Time_SLXS_Gauss_phi_L2.png}
\caption{Convergence of $E_{\phi}$ for SLXS Gauss scheme, for steady state test problem.}
\label{fig:constant_time_gauss_phi}
\end{figure}
Eliminating the temporal error has made it clear that for TRT problems with temperature dependent quantities, SLXS Lobatto converges $E_{\phi} \propto P+1$, and SLXS Gauss converges $E_{\phi} \propto P+2$.  
Similarly, from \fig{fig:constant_time_lobatto_t} and \fig{fig:constant_time_gauss_t} it is clear that SLXS Lobatto converges $E_T \propto P$ and SLXS Gauss converges $E_T \propto P+2$
\begin{figure}[!htp]
\centering
\includegraphics[width=10cm]{chapter6_grey_radtran/Dissertation_Data/Constant_Time_SLXS_Lobatto_temp_L2.png}
...
\begin{figure}[!hbp]
\centering
\includegraphics[width=10cm]{chapter6_grey_radtran/Dissertation_Data/Constant_Time_SLXS_Gauss_temp_L2.png}
\caption{Convergence of $E_{T}$ for SLXS Gauss scheme, for steady state test problem.}
\label{fig:constant_time_gauss_t}
\end{figure}

We now examine the convergence of $E_{\phi_A}$.  The orders of convergence estimated by \fig{fig:constant_time_lobatto_phi_a}, are not identical to those estimated in MMS2 shown in \fig{fig:mms3_slxs_lobatto_phi_a}.  
...
Similarly the convergence of $E_{\phi_A}$ for SLXS Gauss in the steady state problem, given in \fig{fig:constant_time_gauss_phi_a}, is not identical to the estimates of \fig{fig:mms3_slxs_gauss_phi_a}.
\begin{figure}[!htp]
\centering
...
Finally, we consider the convergence of $E_{T_A}$.  If we were to liken the neutron transport interaction rate, $IR$, to the thermal radiative transfer temperature, $T$, then from \fig{fig:constant_time_lobatto_t_a} the SLXS Lobatto thermal radiative transfer spatial convergence of $T$ quantities is one order less than the SLXS Lobatto neutron transport convergence of $IR$ quantities, as SLXS Lobatto converges $E_{IR_A} \propto 2P$ in \fig{fig:constant_time_lobatto_t_a}.  
Conversely in \fig{fig:constant_time_gauss_t_a}, SLXS Lobatto converges $E_{T_A} \propto 2P+2$, whereas SLXS for neutron transport converges $E_{IR_A} \propto 2P+1$ as seen in \figs{fig:varxs_I_A_p1}{fig:varxs_I_A_p4}.
\begin{figure}[!htp]
\centering
...
The implicit Euler scheme can be written as:
\benum
\label{eq:ie}
\begin{array}{c|c|c}
\hline
...
\end{array} \pep
\eenum
We use the second order scheme of Alexander with,
\benum
...
\end{array} \pec
\eenum
and the three stage, third order scheme:
\benum
...
\eeanum
$t \in[0,1]$, $\sigma_s = 0.1$, $\sigma_a = 2.5$, $C_v = 0.2$, $x\in[0,10]$ discretized with 10 equally spaced cells.  Convergence of $E_{\phi}$ as a function of time step size for IE, 2-2, and 3-3 time differencing schemes are given in \fig{fig:e_phi_time}.
\begin{figure}[!htp]
\centering
...
\end{figure}
As expected, the implicit Euler scheme converges 1st order in time, the second order, two stage Alexander-2-2 scheme converges second order, and the Alexander 3-3 scheme converges third order in time.
The same data for temperature is given in \figs{fig:big_dt}{fig:small_dt}.  Surprisingly, it took a very large number of time steps for the 2-2 and 3-3 schemes to become asymptotic, thus \fig{fig:big_dt} shows the convergence of $E_{T}$ for larger time steps, and \fig{fig:small_dt} shows convergence of $E_{T}$ for smaller time steps, when the 2-2 and 3-3 schemes begin to demonstrate their respective asymptotic orders of convergence.
\begin{figure}[!htp]
\centering
