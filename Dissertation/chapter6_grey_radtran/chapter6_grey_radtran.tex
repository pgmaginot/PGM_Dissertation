...
The remainder of this chapter will be divided into three sections.  Section \ref{sec:chap6_linearization} will describe our linearization and discretization of the grey TRT equations.  
...
\eenum
We will occasionally refer to \eqt{eq:analytic_t_1}, and subsequent, similar equations, as a temperature update, as \eqt{eq:analytic_t_1} is the non-linear iteration for temperature we must converge in order to solve the grey TRT.
...
\benum
\frac{1}{4\pi} \left( 4\pi  a_{11} \Delta t \sigma_a  D_*  \right) \left(C_v + 4\pi a_{11} \Delta t \sigma_a D_*  \right)^{-1}\sigma_a \phi_1 \pep
\eenum
Defining a constant, $\nu_1$,
\benum
...
Before proceeding, it is critical to note that in \eqt{eq:k_i_vec_example} and \eqt{eq:k_t_discretized} we are approximate the ``true'' spatial dependence of the Planckian, 
\benum
...
\subsection{Attempts at the Su-Olson Problem}
...
Scattering opacity is constant in space and temperature, absorption opacity is constant in space and temperature, and further assuming that $C_v \propto T^3$, a semi-analytic solution can be obtained.
Assuming that $C_v = \alpha T^3$ is critical.
As Long, et al. \cite{alex_paper} noted, this assumption regarding $C_v$ is not physical, but is required to make the thermal radiative transfer equations linear in $I$ and $T^4$, or conversely linear in $I$ and material energy density. 
...
That being said, we have made a fundamental assumption in our thermal radiative transfer discretization, the linearization of the Planckian and nested temperature iteration.  
By linearizing the Planckian, we are fundamentally solving for temperature, not material energy.
Additionally we have assumed that $C_v$ is explicitly non-zero at all times.
However, given the initial temperature condition is explicitly zero, this implies the initial $C_v$ is also zero.
This problematic for our solution methodology.  
A near-zero heat capacity would result in the radiation and material energy densities coming into equilibrium nearly instantly, but a zero heat capacity implies a material that cannot accept heat, and thus can never be heated up.
%Not wanting to entirely redo our solution methodology and subsequent computer science implementation this late in the dissertation to perform a single verification problem, we opted to add define
%$C_v = \alpha T^3 + \beta$, where $\alpha$ is the same as in \cite{su_olson_1}, and $\beta$ is a small, positive offset.
...
In \tbl{tbl:iteration_count}, we give a sampling of the iteration counts required to solve one type of each problem we have considered, both with sweeps alone and with MIP DSA acceleration.
